"""
Method 5: Hybrid Registration - Quick Test Script
Author: [Team Member 5]
Date: 2025

Combines ORB feature matching with MI refinement.
"""
...
print("="*60)
print("Method 5: Hybrid Registration - Quick Test")
print("="*60)
...
print("\n" + "="*60)
print("Initializing Hybrid registration (ORB + MI)...")
print("="*60)
registrator = HybridRegistration(
    use_orb=True,
    nfeatures=2000,
    enable_mi_refinement=True,
    mi_iterations=200
)
print("✅ Hybrid registrator initialized")
...
# Phase 1: Find best matching atlas slice (ORB-based)
print("\n--- Phase 1: ORB Z-Level Search ---")
...
try:
    ...
    # Unpack best result: (filename, match_count, stats)
...
print("\n--- Phase 2: ORB RANSAC + Phase 3: MI Refinement ---")
start_time = time.time()
...
try:
    ...
    if reg_result['success']:
        ...
        print(f"✅ ORB matches: {n_matches}")
        print(f"✅ RANSAC inliers: {n_inliers} ({n_inliers/n_matches*100:.1f}%)")
        print(f"✅ MI refinement: {reg_result.get('refinement', 'None')}")

        if 'mi_stats' in reg_result:
            print(f"✅ MI iterations: {reg_result['mi_stats'].get('iterations', 'N/A')}")

        print(f"✅ Time: {phase23_time:.2f}s")
...
print("\n" + "="*60)
print(f"Total pipeline time: {phase1_time + phase23_time:.2f}s")
print("="*60)
...
print("="*60)
print("\nNext steps:")
print("1. Run batch evaluation: python run_method5_batch.py")
